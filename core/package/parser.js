 ...
 */
function getDependencyType(spec) {
    // Git URLs
 ...
 */
export function parseDependencies(deps, options) {
    if (!deps)
        return [];
    const result = [];
 ...
 */
export function parseScripts(scripts) {
    if (!scripts)
        return {};
    const result = {};
 ...
 */
export function resolveEntryPoint(pkg, options) {
    const result = {
        entry: null,
        main: pkg.main,
    };
    ...
    if (pkg.exports !== undefined) {
        const exportResult = resolveExportsField(pkg.exports, options?.subpath ?? '.', options?.conditions ?? [], options?.type ?? 'commonjs');
 ...
 */
function resolveExportsField(exports, subpath, conditions, type) {
    ...
    if (typeof exports === 'object') {
        ...
        if (isSubpathMap) {
            ...
            return resolveConditionalExport(target, conditions, type);
        }
        else {
            ...
            return resolveConditionalExport(exports, conditions, type);
        }
    }
    return null;
}
 ...
 */
function resolveConditionalExport(conditional, conditions, type) {
 ...
 */
function resolveTypesVersions(typesVersions, tsVersion) {
 ...
 */
export function parseBin(pkg, options) {
    const result = {};
    if (!pkg.bin) {
        return result;
    }
 ...
 */
export function parseKeywords(keywords, options) {
