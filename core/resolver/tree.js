...
export class DependencyTreeBuilder {
    ...
    async resolve(manifest) {
        ...
        await Promise.all(depEntries.map(([name, range]) => this.resolvePackage(name, range, ctx, resolvedNodes, false, false)));
        ...
        if (devDepEntries.length > 0) {
            ...
            await Promise.all(devDepEntries.map(([name, range]) => this.resolvePackage(name, range, ctx, resolvedNodes, true, false)));
        }
     ...
     */
    async resolvePackage(name, range, ctx, resolved, isDev, isOptional, requester = 'ROOT') {
        // Resolve version
        let version;
        try {
        ...
        if (ctx.resolvingStack.has(pkgKey)) {
            ...
            const cycle = [...ctx.resolvingStack, pkgKey].slice([...ctx.resolvingStack].indexOf(pkgKey));
            const cycleNames = cycle.map((k) => k.split('@')[0]).filter((n) => n !== undefined);
        ...
        try {
            ...
            if (regularDeps.length > 0) {
                await parallelLimit(regularDeps, DEFAULT_CONCURRENCY, ([depName, depRange]) => this.resolvePackage(depName, depRange, ctx, resolved, isDev, false, name));
            }
            if (optionalDeps.length > 0) {
                await parallelLimit(optionalDeps, DEFAULT_CONCURRENCY, ([depName, depRange]) => this.resolvePackage(depName, depRange, ctx, resolved, isDev, true, name));
            }
        }
        finally {
     ...
     */
    async resolveVersion(name, range, ctx) {
     ...
     * - Caches results
     */
    async getPackageInfo(name, version, ctx) {
     ...
     */
    checkPeerDependencies(packageName, peerDeps, ctx, resolved) {
     ...
     */
    async autoInstallPeerDeps(ctx, resolved) {
        ...
        ctx.warnings = ctx.warnings.filter((w) => !(w.type === 'peer-missing' && resolved.has(w.peer)));
    }
    /**
     * Check if package is compatible with current platform
     */
    isPlatformCompatible(os) {
    ...
    cloneNodeWithoutNested(node) {
        ...
        if (node.optional !== undefined)
            clone.optional = node.optional;
        if (node.peerDependencies)
            clone.peerDependencies = node.peerDependencies;
        if (node.bundledDependencies)
            clone.bundledDependencies = node.bundledDependencies;
        if (node.hasBundled !== undefined)
            clone.hasBundled = node.hasBundled;
        if (node.circularTo)
            clone.circularTo = node.circularTo;
        if (node.integrity !== undefined)
            clone.integrity = node.integrity;
        if (node.resolved !== undefined)
            clone.resolved = node.resolved;
        return clone;
    }
    /**
     ...
     */
    hoistDependencies(resolved, ctx) {
     ...
     */
    generateResolvedUrl(name, version) {
 ...
 */
export function detectCircularDependencies(tree) {
    const cycles = [];
